...
function toggleCollapse(header) {
    ...
    if(wrap && wrap.classList.contains('collapsible-wrap')) {
...
function handleTimerTitleClick(id) {
    ...
    if (timer.storeName) { 
        document.getElementById('timer-action-title').innerText = timer.storeName; 
        document.getElementById('timer-action-id').value = id; 
        document.getElementById('timer-action-modal').classList.add('active'); 
    } else { 
        openStoreModal(id); 
    }
}
...
function editOrderNumber() {
    ...
    if(timer) {
        closeModal('timer-action-modal');
        openOrderNumberModal('修改單號', timer.orderNumber || '', function(newNum) {
            timer.orderNumber = newNum;
            localStorage.setItem(getStoreKey('order_active_timers'), JSON.stringify(activeTimers));
            renderActiveTimers();
        });
    }
...
function cloneTimer() {
    ...
    if(timer) {
        closeModal('timer-action-modal');
        const nextNum = incrementOrderNumber(timer.orderNumber);
        openOrderNumberModal(`為【${timer.storeName}】新增訂單`, nextNum, function(orderNum) {
            const now = Date.now(); 
            activeTimers.push({ id: 'timer_' + now + '_' + Math.random().toString(36).substr(2, 5), startTime: now, storeName: timer.storeName, orderNumber: orderNum });
            localStorage.setItem(getStoreKey('order_active_timers'), JSON.stringify(activeTimers)); 
            renderActiveTimers(); 
            checkWaitState();
        });
...
function rebindTimer() { 
    const id = document.getElementById('timer-action-id').value; 
    closeModal('timer-action-modal'); 
    openStoreModal(id); 
}
...
async function addCustomStore() {
    ...
    if (name && name.trim() !== '') {
        customStores.push({ code: '自訂', name: name.trim() }); localStorage.setItem('app_custom_stores', JSON.stringify(customStores));
        searchStore(); selectStore(name.trim());
    }
}
...
function selectStore(storeName) {
    ...
    openOrderNumberModal(`已選擇：${storeName}\n請輸入訂單號碼`, '', async function(orderNum) {
        timer.storeName = storeName; 
        timer.orderNumber = orderNum; 
        const unboundTimers = activeTimers.filter(t => !t.storeName && t.id !== id);
        if (unboundTimers.length > 0) {
            if (await appConfirm(`您還有 ${unboundTimers.length} 個未綁定的計時器。\n是否要自動套用【${storeName}】並自動遞增單號？`, '自動綁定')) {
                let currentNum = timer.orderNumber; 
                unboundTimers.forEach(ut => { 
                    currentNum = incrementOrderNumber(currentNum); 
                    ut.storeName = storeName; 
                    ut.orderNumber = currentNum; 
                });
            }
        }
        localStorage.setItem(getStoreKey('order_active_timers'), JSON.stringify(activeTimers)); 
...
async function editIncomeAmount(id) {
    ...
    if (newAmtStr !== null && newAmtStr.trim() !== '') {
        const numAmt = Number(newAmtStr);
        if (isNaN(numAmt) || numAmt < 0) {
            return await appAlert('請輸入有效的數字', '輸入錯誤');
        }
        record.amount = numAmt;
        localStorage.setItem(getStoreKey('order_history_records'), JSON.stringify(historyRecords));
        renderWeeklyData(); 
...
function importData(event) { 
    ...
    reader.onload = async function(e) { 
        try { 
            const data = JSON.parse(e.target.result); let valid = false; 
            Object.keys(data).forEach(k => { if(k.includes('order_') || k.includes('app_')) { localStorage.setItem(k, data[k]); valid = true; } }); 
            if(valid) { await appAlert('資料匯入成功！即將重新載入頁面。', '匯入成功'); location.reload(); } 
            else await appAlert('無效的備份檔案格式。', '匯入失敗'); 
        } catch(err) { await appAlert('匯入失敗：檔案損毀或格式錯誤。', '錯誤'); } 
    }; 
